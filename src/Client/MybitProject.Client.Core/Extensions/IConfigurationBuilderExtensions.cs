...
public static partial class IConfigurationBuilderExtensions
{
    ...
    public static IConfigurationBuilder AddClientConfigurations(this IConfigurationBuilder builder, string clientEntryAssemblyName)
    {
        ...
        var sharedAssembly = Assembly.Load("MybitProject.Shared");

        configBuilder.AddJsonStream(sharedAssembly.GetManifestResourceStream("MybitProject.Shared.appsettings.json")!);

        var envSharedAppSettings = sharedAssembly.GetManifestResourceStream($"MybitProject.Shared.appsettings.{AppEnvironment.Current}.json");
        if (envSharedAppSettings != null)
        {
            configBuilder.AddJsonStream(envSharedAppSettings);
        }

        var clientCoreAssembly = AppDomain.CurrentDomain.GetAssemblies().Single(asm => asm.GetName()?.Name is "MybitProject.Client.Core");

        configBuilder.AddJsonStream(clientCoreAssembly.GetManifestResourceStream("MybitProject.Client.Core.appsettings.json")!);

        var envClientCoreAppSettings = clientCoreAssembly.GetManifestResourceStream($"MybitProject.Client.Core.appsettings.{AppEnvironment.Current}.json");
